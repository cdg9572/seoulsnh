<?php
    ...
    $data2 = $C_Conversion->conversion_Month_List(array_merge($_GET,$_POST,$args));    
    $data_list2 		= $data2['data'];
    $data_list_cnt2 	= count($data_list2);

    $args['cv_type'] = "call";	
    $data3 = $C_Conversion->conversion_Month_List(array_merge($_GET,$_POST,$args));    
    $data_list3 		= $data3['data'];
    $data_list_cnt3 	= count($data_list3);

    $args['cv_type'] = "kakao";	
    $data4 = $C_Conversion->conversion_Month_List(array_merge($_GET,$_POST,$args));    
	...
	for ($i = 0 ; $i < $data_list_cnt ; $i++) {    
        ...
        for ($j = 0 ; $j < $data_list_cnt2 ; $j++) {                                            
            if($data_list[$i]['StatusDay'] == $data_list2[$j]['StatusDay']){
                $loaction_sum += $data_list2[$j]['cnt'];
             }
        }   

        for ($j = 0 ; $j < $data_list_cnt3 ; $j++) {                                            
            if($data_list[$i]['StatusDay'] == $data_list3[$j]['StatusDay']){
                $call_sum += $data_list3[$j]['cnt'];	
             }
        }   

        for ($j = 0 ; $j < $data_list_cnt4 ; $j++) {                                            
            if($data_list[$i]['StatusDay'] == $data_list4[$j]['StatusDay']){
						...
						<table>
							...
							<tbody>
								<tr>
									...
									<td><?=$arr_tmp[1]?></td>
                                    <td><?=$loaction_sum?></td>
                                    <td><?=substr((($loaction_sum / $arr_tmp[1]) * 100),0,4);?>%</td>
                                    <td><?=$call_sum?></td>
                                    <td><?=substr((($call_sum / $arr_tmp[1]) * 100),0,4);?>%</td>
                                    <td><?=$kakao_sum?></td>
                                    <td><?=substr((($kakao_sum / $arr_tmp[1]) * 100),0,4);?>%</td>									
								</tr>
								<?
										$dummy = 1;
										for ($i = 0 ; $i < $data_list_cnt ; $i++) {
