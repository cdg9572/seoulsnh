<?php
	...
	$args = array();
    $args = array("show_row" => 20 , "pagetype" => "admin","jb_code_in" => $_SESSION['adminbbs'] );	    
	$data = "";
...
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		<?php include_once($GP -> INC_ADM_PATH."/header.php"); ?>
						...
						<tbody>
							...
							for ($i = 0 ; $i < $data_list_cnt ; $i++) {
								...
								$jb_code 					= $data_list[$i]['jb_code'];
									...
									$write_btn = $C_Button -> getButtonDesign('type2','글쓰기',0,"javascript:BoardAdminWrite(" . $jb_code .")", 50,'');
								
									$edit_btn = $C_Button -> getButtonDesign('type2','보기',0,"javascript:BoardAdminWin(" . $jb_code .")", 50,'');
								
									if ($_SESSION['suserlevel'] == '9') {
										$edit_btn .= $C_Button -> getButtonDesign('type2','설정',0,"layerPop('ifm_reg','./bbs_edit.php?jb_code=" . $jb_code. "', '100%', 1100)", 50,'');
									}
		...
		<?php include_once($GP -> INC_ADM_PATH."/footer.php"); ?>
	</div>
</div><!-- 전체를 감싸는 Wrap //-->
</body>
</html>
...
function BoardAdminWrite(jb_code)
{
	var url="/bbs/index.php?jb_code=" + jb_code + "&jb_mode=twrite";
	window.open (url,'board_admin_win' ,'scrollbars=yes, top=0, left=0, width=1100, height=900')
}
</script>