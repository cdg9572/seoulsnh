...
//변수에 할당하기
$rst_board = $C_board->DESC_BOARD_LIST();
...
for($i=0; $i<count($rst_board); $i++) {    
    ...
    ${$rst_board[$i]['Field']}  = $_POST[$rst_board[$i]['Field']]  ;   
...
$jb_notice_check = $_POST['jb_notice_check'] ;
$jb_etc1 = $_POST['jb_etc1'] ;
$jb_etc2 = $_POST['jb_etc2'] ;
$jb_mobile =  $_POST['jb_mobile1']. "-" .  $_POST['jb_mobile2'] . "-" . $_POST['jb_mobile3'];
$jb_email =  $_POST['jb_email'];
if($jb_code == "60" || $jb_code == "70"){
    $jb_email =  $_POST['jb_email1'] ."@".  $_POST['jb_email2'];
 }
...
//비밀번호 입력확인
if($check_id != '' or $jb_secret_check = "Y") {
...
// 비밀번호 수정할 경우
//if($jb_secret_check == "Y"){$args['jb_password'] = $jb_password;}

//업로드 파일 갯수
$file_cnt=is_null($_FILES['jb_file']['tmp_name']) ? 0 : count($_FILES['jb_file']['tmp_name']);
...
if($del_file_front == "Y") {
	...
	if ($is_fileName) {
		$args_f = "";
		$args_f = array("forder" => $file_movie_path , "files" => $_FILES[$file_orName], "max_file_size" => 1024 * 307200, "able_file" => array() );	
	...
	$insertFileCheck	= false;
		if ($is_fileName) {
			$args_f = "";
			$args_f = array("forder" => $file_movie_path , "files" => $_FILES[$file_orName], "max_file_size" => 1024 * 1000, "able_file" => array() );
...
for($i=0; $i<count($rst_board); $i++) {
	...
	//if($jb_secret_check != "Y"){if($rst_board[$i][Field]=="jb_password") continue;    }	
	if($rst_board[$i]['Field']=="jb_file_name") continue;
    if($rst_board[$i]['Field']=="jb_file_code") continue;
    if(!$jb_front_image){if($rst_board[$i]['Field']=="jb_front_image") continue;    }	
    //if($rst_board[$i]['Field']=="jb_front_image") continue;
...
{
	$args = "";
    $args = array("jb_code" => $jb_code , "jb_idx" => $jb_idx, "keys_values" => $keys_values );	   
	$result_key = $C_board->BOARD_UPDATE($args);	
	
	
	$args = "";
	$args = array("jb_code" => $jb_code , "jb_idx" => $jb_idx, "jb_content" => $jb_content );	
	
	$result_key = $C_board->BOARD_DETAIL_UPDATE($args);	
}
...
$row_del = $C_board->BOARD_FILE_CONTENT($args);
...
$args = "";
$args = array("jb_file_name" => $real_file_names , "jb_file_code" => $new_file_names );	
//====================================================================================================== 다중파일업로드 관련 처리 End
...
if($img_full_name != "") {
	...
	//$args = array("jb_img_code" => $img_name );	
}

	
$args = array("jb_code" => $jb_code , "jb_idx" => $jb_idx,"jb_file_name" => $real_file_names , "jb_file_code" => $new_file_names,"jb_img_code" => $img_name  );	
$file_update = $C_board->BOARD_FILE_UPDATE($args);	
