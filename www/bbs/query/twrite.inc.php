...
$jb_secret_check = $_POST['jb_secret_check'] ;
$jb_password = $_POST['jb_password'] ;
...
$rst = $C_board->Board_Depth_Max($args);
...
//영상 썸네일 
		...
		if ($is_fileName) {
			$args_f = "";
			$args_f = array("forder" => $file_movie_path , "files" => $_FILES[$file_orName], "max_file_size" => 1024 * 1000, "able_file" => array("jpg","gif","png","bmp") );			
			$C_Fileup = new Fileup($args_f);
			$updata		= $C_Fileup -> fileUpload();
...
//이메일 보내기
// if($jb_code == "20"){
// 	 $receive_email	= "cdg9572@gmail.com";							
// 	 $receive_name		= "";	
// 	 $email_subject = $toq_hospital . "온라인 상담에 글이 등록 되었습니다.";   
// 	 $sender_email = "umlover9@gmail.com";
// 	 $sender_name = "";
...
// 	 $contents= "제목 : " . trim($jb_title)."<br>이름 : " . trim($jb_name)."<br>이메일 : " . trim($jb_email)."<br>내용 : " . trim($jb_content)."<br>";    	
...
if($keys && $values)
{
	...
	$result_key = $C_board->BOARD_WRITE($args);		
}
...
if($result_key)
{
	...
	$keys="";
	$values="";
	$rst_detail = $C_board->DESC_BOARD_DETAIL();
	...
	if($keys && $values)
	{
		...
		$result_key_detail = $C_board->BOARD_WRITE_DETAIL($args);		
	}
}
