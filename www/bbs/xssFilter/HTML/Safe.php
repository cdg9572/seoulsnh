...
class HTML_Safe
{
     ...
     * @var array
     */
    public $singleTags = array('area', 'br', 'img', 'input', 'hr', 'wbr', 'param', );
     ...
     * @var array
     */
    public $attributes = array('dynsrc', 'id', );
    ...
    public function openHandler(&$parser, $name, $attrs)
    {
		...
		if($name == 'param')
		{
				// param 태그의 내용이 보안 관련 내용이라면 제거
				if(in_array(strtolower($attrs['name']), array('allowscriptaccess', 'allownetworking', 'invokeurls', 'enablehref', 'enablejavascript', 'nojava')))
				{
					return true;
				}
		}
		...
		if($name == 'embed') {
			...
			{
				$temp = strtolower($key);
				if(in_array($temp, array('allowscriptaccess', 'allownetworking', 'invokeurls', 'enablehref', 'enablejavascript', 'nojava')))
				{
					unset($attrs[$key]);
				}
			}
			$attrs['allowScriptAccess'] = 'never';
			$attrs['allowNetworking'] = 'internal';
			$attrs['invokeURLs'] = 'false';
			$attrs['enablehref'] = 'false';
			$attrs['enablejavascript'] = 'false';
			$attrs['nojava'] = 'true';
		}
		...
		if($name == 'object')
		{
			/* 오브젝트 태그일 때 */
			$this->xhtml .= '<param';
			$this->writeAttrs(array('name' => 'allowScriptAccess'));
			$this->writeAttrs(array('value' => 'never'));
			$this->xhtml .= ' />';
			$this->xhtml .= '<param';
			$this->writeAttrs(array('name' => 'allowNetworking'));
			$this->writeAttrs(array('value' => 'internal'));
			$this->xhtml .= ' />';
			$this->xhtml .= '<param';
			$this->writeAttrs(array('name' => 'invokeURLs'));
			$this->writeAttrs(array('value' => 'false'));
			$this->xhtml .= ' />';
			$this->xhtml .= '<param';
			$this->writeAttrs(array('name' => 'enablehref'));
			$this->writeAttrs(array('value' => 'false'));
			$this->xhtml .= ' />';
			$this->xhtml .= '<param';
			$this->writeAttrs(array('name' => 'enablejavascript'));
			$this->writeAttrs(array('value' => 'false'));
			$this->xhtml .= ' />';
			$this->xhtml .= '<param';
			$this->writeAttrs(array('name' => 'nojava'));
			$this->writeAttrs(array('value' => 'true'));
			$this->xhtml .= ' />';
		}
