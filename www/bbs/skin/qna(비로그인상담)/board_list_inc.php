...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
        ...
        if($check_level < 9 && $data_list[$i]['jb_secret_check'] == "Y")        
		{
			...
			if($jb_mb_id == "") {                
				...
				if($check_id != $jb_mb_id) {
                    ...
                    $args = array("jb_code" => $jb_code , "jb_group" => $jb_group );                   
					$rst = $C_board -> MEM_SECRET_CHECK($args);		
		...
		//검색배열시 -by Shin//
		if(is_array($_GET["search_key"])) {
			if($_GET['search_key']['jb_name']) $s_key[] = "search_key[jb_name]=".$_GET['search_key']['jb_name'];
			if($_GET['search_key']['jb_title']) $s_key[] = "search_key[jb_title]=".$_GET['search_key']['jb_title'];
			if($_GET['search_key']['jb_content']) $s_key[] = "search_key[jb_content]=".$_GET['search_key']['jb_content'];
			$get_par = str_replace("&search_key=Array","&".implode("&",$s_key),$get_par);
		}
		//검색배열끝//
