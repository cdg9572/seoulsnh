				...
				<div class="tableType-01 green no-border">
                    ...
                    <? if($jb_step ==  0 or $check_level == 9 ){?>
                        ...
                        <? if($check_level >= $db_config_data['jba_reply_level']){?>
                            <a href='#none' onclick="javascript:location.href='<?=$get_par?>&jb_mode=treply'" class='btn bg-red'>답글</a>
                        <?}?>
                        <?if($check_level >= $jb_mb_level && $check_level >= $db_config_data['jba_write_level']) {?>
						<a href="#none" onclick="javascript:location.href='<?=$get_par?>&jb_mode=tdelete'"  class="btn bg-gray" title="삭제">삭제</a>
						<a href="#\" onclick="javascript:location.href='<?=$get_par?>&jb_mode=tmodify'"  class="btn bg-gray" title="수정">수정</a>
						<?} }?>
						<a href="<?=$index_page?>?jb_code=<?=$jb_code?>&<?=$search_key?>&search_keyword=<?=$search_keyword?>&page=<?=$page?>" class="btn bg-black">목록</a>
					</div>
