...
if($check_level < 9) {	
	...
	$rst = $C_board->Board_See_Up($args);	
}

$args = '';
$args = array("jb_code" => $jb_code , "jb_idx" => $jb_idx );	
$board_data = $C_board->Board_Read_Data($args);
...
$board_data_jb_F = $C_board->Board_Read_Data_B_A($args);
...
if($board_data) {
	...
	$be_idx = $board_data_jb_F[0]['jb_idx'];
	$af_idx = $board_data_jb_F[1]['jb_idx'];	
	
	$be_content	= $C_Func->dec_contents_edit($board_data_jb_F[0]['jb_name']);
	$be_content = strip_tags($be_content);
	$be_content = $C_Func->strcut_utf8($be_content, 52, false, "...");		
		
	$af_content	= $C_Func->dec_contents_edit($board_data_jb_F[1]['jb_title']);
