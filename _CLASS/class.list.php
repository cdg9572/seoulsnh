...
CLASS ListClass
{
	...
	function listInfo ($args) {
        ...
        //print_r($qry_get);
        // echo "echo:"; print_r($qry_get); echo "<br>" ;
	...
	function getCounts () {
		...
		($show_row)? $totalpages	= ceil($total / $show_row) : "" ;		
		($show_page)?    $startpage 	= ((ceil(($page / $show_page) - 0.01) - 1) * $show_page) + 1: "" ;		
	...
	{		
		...
		{			
			...
			$rst ='<a href="?page=1&' . $tail . '" title="처음으로 이동하기" class="page-btn prev"><span><img src="/resource/images/page-all-left.png" width="100%" alt=""></span></a> ';
			$rst .='<a href="?page='  . $prev_page . '&' . $tail . '" title="이전으로 이동하기" class="page-btn prev"><span><img src="/resource/images/page-left.png" width="100%" alt=""></span></a> ';
			for($i = $start ; $i <= $end ; $i++){
				$pageLastclass = "";
				if ($i>=10) {
					$pageLastclass = "class=\"page_Last\"";
				}				
				$rst .= ($i != $page)? "<a href='?page=" . $i . '&' . $tail . "' class='page-btn'>":"<strong class='page-btn current' title='현재 페이지'>";
				$rst .= ($i == $page)? "$i" : "$i";
				$rst .= ($i != $page)? "</a>":"</strong>";
				$rst .= ($i != $end) ? " " : "";
			}
			$rst .= ' <a href="?page=' . $next_page . '&'. $tail . '" title="다음 페이지 이동하기" class="page-btn next"><span><img src="/resource/images/page-right.png" width="100%" alt=""</span></a>';
			$rst .= ' <a href="?page=' . $this -> page_info['tpage'] . '&'. $tail . '" title="마지막 페이지 이동하기" class="page-btn next"><img src="/resource/images/page-all-right.png"" width="100%" alt=""></a>';

		} else {			
			$rst ='<a href="?page=1&' . $tail . '" title="처음으로 이동하기" class="page-btn prev"><span><img src="/resource/images/page-all-left.png" width="100%" alt=""></span></a> ';
			$rst .='<a href="?page=1&'. $tail . '" title="이전으로 이동하기" class="page-btn prev"><span><img src="/resource/images/page-left.png" width="100%" alt=""></span></a> ';
			$rst .='<a href="?page=1&'. $tail . '" class="page-btn current">1</a>';			
			$rst .= ' <a href="?page=1&'. $tail . '" title="다음 페이지 이동하기" class="page-btn next"><span><img src="/resource/images/page-right.png" width="100%" alt=""</span></a>';
			$rst .= ' <a href="?page=1&'. $tail . '" title="마지막 페이지 이동하기" class="page-btn next"><img src="/resource/images/page-all-right.png" width="100%" alt=""></a>';
	...
	//echo "===========================================".$tail;
        ...
        $type = ($_GET['tm_type'])? "tm_type=".$_GET['tm_type'].'&':"";
		if($total > 0)
		{
			$rst = '<li class="boxPagingPrev"><span><a href="?'. $type .'page='  . $prev . '&' . $tail . '"><strong>&lt;</strong></a></span></li><li class="boxPagingNum">';
			...
			for($i = $start ; $i <= $end ; $i++){
				...
				$rst .= ($i != $page)? "<a href='?".$type."page=" . $i . '&' . $tail . "' $pageLastclass>":"";
				$rst .= ($i == $page)? "<strong>$i</strong>" : "".$i."";
				$rst .= ($i != $page)? "</a>":"";
				$rst .= ($i != $end) ? " " : "";
			}
			$rst .= '</li><li class="boxPagingNext"><span><a href="?'. $type .'page=' . $this -> page_info['next'] . '&'. $tail . '"><strong>&gt;</strong></a></span></li>';
	...
	function getExcel ($args = '') {
		...
		for($i = 0; $i < $rstCnt2; $i++)
		{
			...
			for($j = 0 ;$j < $rstCnt1 ; $j++)
			{
				...
				if ($get_column_data && !preg_replace('\^', $get_column_data)) {
					$data_str = $get_column_data;
				}else{
					if (preg_replace('\^', $column_name)) {
						$get_and_arr = explode('^', $column_name);
						foreach ($get_and_arr as $k2 => $v2) {
							//$add_sep = (preg_replace("'",$v2)) ? str_replace("'","",$v2) : '';
