...
CLASS Sms extends Dbconn
{
	...
	function Sms_Send_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " ssl_senddate BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
		...
		$args['q_idx'] = "ssl_idx";
		$args['q_col'] = "*";
		$args['q_table'] = " tblSmsSendList_" . $year . " SL LEFT OUTER JOIN tblSmsAccount SA ON SL.ssa_idx=SA.ssa_idx";
		$args['q_where'] = $addQry;
	...
	function Sms_Client_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " ssa_regdate BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
	...
	// auth  : JH 2013-09-16 월요일
	// param
	function Sms_History_List ($args = '') {
		global $C_Func;
		if (is_array($args)) foreach ($args as $k => $v) ${$k} = $v;
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " tsh_reg_date BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
		...
		$args['q_idx'] = "tsh_idx";
		$args['q_col'] = "*";
		$args['q_table'] = "  tblSmsHistory";
		$args['q_where'] = $addQry;
		$args['q_order'] = "tsh_reg_date desc";
		$args['q_group'] = "";
	...
	function Mem_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " mb_register_date BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
	...
	function Group_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " gr_regdate BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
	...
	// auth  : JH 2013-09-16 월요일
	// param
	function Sms_Client_History($args = '') {
		if (is_array($args)) foreach ($args as $k => $v) ${$k} = $v;
	
		if($tsh_cnt) $ssa_num = $tsh_cnt;
	
		$qry = "
			insert into tblSmsHistory
			(
			tsh_name,
			tsh_id,
			tsh_mobile,
