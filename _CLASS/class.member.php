...
{	
	...
	function Mem_Out_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			...
			$addQry .= " mb_withdrawal_date BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
	...
	function membersFindInfo2 ($args) {
		...
		//탈퇴 회원은 제외
		$qry = "Select mb_code, mb_email, mb_name, mb_id From tblMember Where mb_id = '$mb_id' and mb_email = '$mb_email' And mb_del_flag = 'N' and mb_status='M' ";
		...
		if($rstMem){
            ...
            $rst = array(
                "mb_code" => $rstMem['mb_code'] , 
                "mb_name" => $rstMem['mb_name'] , 
                "mb_email" => $rstMem['mb_email'] , 
                "mb_id" => $rstMem['mb_id'] , 
                "new_pw" => $new_pw               
            ) ;              
		} else {
	...
	function ID_DobuleCheck($args = '') { 
		...
		$qry = "
			SELECT count(*) as cnt FROM tblMember where mb_id='$mb_id' 
		";
		$rst = $this -> DB -> execSqlOneRow($qry);
		return $rst;	
	}	
