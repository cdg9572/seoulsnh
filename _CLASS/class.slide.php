...
CLASS Slide extends Dbconn
{
    ...
    function TO_AUTO_CHAGE($args) {
		...
		for($i=0; $i<count($arr_tmp); $i++) {
			$idx = $arr_tmp[$i];			
			$qry = " update tblSlide set ts_desc = '$max_desc' where ts_idx = '$idx'	";			
	...
	function Main_Slide_Show($args='') {
		...
		//$addQry .= " AND ('".date('Y-m-d')."' BETWEEN ts_start_date AND ts_end_date) ";
        
		
		$qry = "
			select * from tblSlide where ts_show ='Y' $addQry order by ts_desc desc, ts_regdate asc $limit
        "; 
	...
	function Slide_List ($args = '') {
		...
		$args['q_where'] = $addQry;
		$args['q_order'] = "ts_desc desc, ts_regdate desc";
		$args['q_group'] = "";
		$args['tail'] = "s_date=" . $s_date . "&e_date=" . $e_date ."&serach_key=" . $search_key . "&search_content=" . $search_cotent . "&tt_cate=" . $tt_cate . "&ts_type=" . $ts_type;
		$args['q_see'] = "";
