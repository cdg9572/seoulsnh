...
CLASS Conversion extends Dbconn
{
	...
	// auth  : JH 2013-09-16 월요일
	// param
	function conversion_Year_List ($args = '') {
		global $C_Func;
		if (is_array($args)) foreach ($args as $k => $v) ${$k} = $v;
		...
		$tail = "";		
		$addQry = " 1=1 
			AND DATE_FORMAT(cv_reg_date,'%Y') = '$Year'
		";		
		
		if($cv_type) {
			$addQry .= "AND cv_type = '$cv_type'";	
		}
		...
		if($excel_file != '') {
			$args['q_col'] = " 
				DATE_FORMAT(cv_reg_date,'%m') As StatusMonth, 
				COUNT(cv_reg_date) as cnt 
			";
		}else{
			$args['q_col'] = " DATE_FORMAT(cv_reg_date,'%m') As StatusMonth, COUNT(cv_reg_date) as cnt ";
		}
		
		$args['q_table'] = "tblConversion";
		$args['q_where'] = $addQry;
		$args['q_order'] = "StatusMonth asc";
		$args['q_group'] = " DATE_FORMAT(cv_reg_date,'%m') ";
	...
	// auth  : JH 2013-09-16 월요일
	// param
	function conversion_Month_List ($args = '') {
		global $C_Func;
		if (is_array($args)) foreach ($args as $k => $v) ${$k} = $v;
		...
		$addQry = " 1=1 
			AND DATE_FORMAT(cv_reg_date,'%Y') = '$Year'
			AND DATE_FORMAT(cv_reg_date,'%m') = '$Month'
		";		
		...
		if($excel_file != '') {
			$args['q_col'] = " 
				DATE_FORMAT(cv_reg_date,'%Y-%m-%d') As StatusDay, 
				COUNT(cv_reg_date) as cnt 
			";
		}else{
			$args['q_col'] = " DATE_FORMAT(cv_reg_date,'%d') As StatusDay, COUNT(cv_reg_date) as cnt ";
		}
		$args['q_table'] = "tblConversion";
		$args['q_where'] = $addQry;
		$args['q_order'] = "StatusDay asc";
		$args['q_group'] = " DATE_FORMAT(cv_reg_date,'%d') ";
