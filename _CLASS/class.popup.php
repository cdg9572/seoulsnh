...
CLASS Popup extends Dbconn
{
    ...
    function TO_AUTO_CHAGE($args) {
		...
		for($i=0; $i<count($arr_tmp); $i++) {
			$idx = $arr_tmp[$i];			
			$qry = " update tblPopUp set pop_desc = '$max_desc' where pop_idx = '$idx'	";			
    ...
    function PopUp_Show() {
		...
		$qry = "
			SELECT * FROM tblPopUp WHERE pop_use='Y' AND ('".date('Y-m-d')."' BETWEEN pop_start_date AND pop_end_date) ORDER BY pop_desc desc, pop_idx ASC
		";		
	...
	function Popup_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			...
			$addQry .= " pop_reg_date BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
        }
        ...
        if ($pop_use == "Y") {
            ...
            $addQry .= " pop_use='Y' AND ('".date('Y-m-d')."' BETWEEN pop_start_date AND pop_end_date) ";

            $args['q_order'] = "pop_desc desc, pop_idx ASC";            
        }
        else{
