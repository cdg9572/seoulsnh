...
CLASS Online extends Dbconn
{
	...
	function Phone_Counsel_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			if ($addQry)
			$addQry .= " AND ";
			$addQry .= " TKR.tfc_regdate BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:59'";
		}
	...
	function Online_Reserve_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			...
			$addQry .= " TOR.tor_del_flag BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
	...
	function Online_Qna_List ($args = '') {
		...
		if (($s_date && $e_date) && ($s_date <= $e_date)) {
			...
			$addQry .= " toq_regdate BETWEEN '$s_date 00:00:00' AND '$e_date 23:59:00'";
		}
	...
	function Online_E_List ($args = '') {
		...
		$args['tail'] = "toq_type=" . $toq_type . "&s_date=" . $s_date . "&e_date=" . $e_date ."&serach_key=" . $search_key . "&search_content=" . $search_cotent;
		$args['q_see'] = "";
		return $C_ListClass -> listInfo($args);
    }
    
    function Online_Info_Del($args) {
		if (is_array($args)) foreach ($args as $k => $v) ${$k} = $v;
		
		
		$qry = "
			delete from tblOnlineQna where toq_idx = '$toq_idx'
		";
		$rst =  $this -> DB -> execSqlUpdate($qry);
		return $rst;
	}
	...
	// auth  : JH 2013-09-13
	// param
	function Online_Info_Modify($args) {
		if (is_array($args)) foreach ($args as $k => $v) ${$k} = $v;
