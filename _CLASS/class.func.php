...
*/
	...
	function makeSelect()
	{
		...
		$count_arr = is_null($selArr) ? 0 : count($selArr);		

		if($count_arr  && is_array($selArr))
		{
	...
	{
		if(preg_match ("[^a-zA-Z0-9]",$str)) return 0;
		return 1;
	}
	...
	function ismail( $str )
	{
		if( preg_match ("([a-z0-9\_\-\.]+)@([a-z0-9\_\-\.]+)", $str) ) return 1;
		return 0;
	}
  ...
  // 상태 select array 받아오기
	...
	function auto_link($str)
	{
		...
		$str = str_replace("&(quot|gt|lt)","!\\1",$str);
		$str = str_replace("([ ]*)target=[\"'_a-z,A-Z]+","", $str);
		$str = str_replace("([ ]+)on([a-z]+)=[\"'_a-z,A-Z\?\.\-_\/()]+","", $str);
		
		// html사용시 link 보호
		$str = str_replace("<a([ ]+)href=([\"']*)($regex[http])([\"']*)>","<a href=\"\\4_orig://\\5\" target=\"_blank\">", $str);
		$str = str_replace("<a([ ]+)href=([\"']*)mailto:($regex[mail])([\"']*)>","<a href=\"mailto:\\4#-#\\5\">", $str);
		$str = str_replace("<img([ ]*)src=([\"']*)($regex[http])([\"']*)","<img src=\"\\4_orig://\\5\"",$str);
		
		// 링크가 안된 url및 email address 자동링크
		$str = str_replace("($regex[http])","<a href=\"\\1\" target=\"_blank\">\\1</a>", $str);
		$str = str_replace("($regex[mail])","<a href=\"mailto:\\1\">\\1</a>", $str);
		
		// 보호를 위해 치환한 것들을 복구 
		$str = str_replace("!(quot|gt|lt)","&\\1",$str);
		$str = str_replace("http_orig","http", $str);
		$str = str_replace("#-#","@",$str);
		
		// link가 2개 겹쳤을때 이를 하나로 줄여줌
		$str = str_replace("(<a href=([\"']*)($regex[http])([\"']*)+([^>]*)>)+<a href=([\"']*)($regex[http])([\"']*)+([^>]*)>","\\1", $str);
		$str = str_replace("(<a href=([\"']*)mailto:($regex[mail])([\"']*)>)+<a href=([\"']*)mailto:($regex[mail])([\"']*)>","\\1", $str);
	...
	function url_auto_link($str)
	{
			...
			$str = str_replace("`(?:(?:(?:href|src)\s*=\s*(?:\"|'|)){0})((http|https|ftp|telnet|news|mms)://[^\"'\s()]+)`", "<A HREF=\"\\1\" TARGET='_blank'>\\1</A>", $str);
			$str = str_replace("/(^|[\"'\s(])(www\.[^\"'\s()]+)/i", "\\1<A HREF=\"http://\\2\" TARGET='_blank'>\\2</A>", $str);
			$str = str_replace("/[0-9a-z_-]+@[a-z0-9._-]{4,}/i", "<a href='mailto:\\0'>\\0</a>", $str);
			$str = str_replace(array("\t_nbsp_\t", "\t_lt_\t", "\t_gt_\t"), array("&nbsp;", "&lt;", "&gt;"), $str);
	
			/*
			// 속도 향상 031011
			$str = str_replace("/&lt;/", "\t_lt_\t", $str);
			$str = str_replace("/&gt;/", "\t_gt_\t", $str);
			$str = str_replace("/&amp;/", "&", $str);
			$str = str_replace("/&quot;/", "\"", $str);
			$str = str_replace("/&nbsp;/", "\t_nbsp_\t", $str);
			$str = str_replace("/([^(http:\/\/)]|\(|^)(www\.[^[:space:]]+)/i", "\\1<A HREF=\"http://\\2\" TARGET='{$config['cf_link_target']}'>\\2</A>", $str);
			//$str = str_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,]+)/i", "\\1<A HREF=\"\\2\" TARGET='$config['cf_link_target']'>\\2</A>", $str);
			// 100825 : () 추가
			// 120315 : CHARSET 에 따라 링크시 글자 잘림 현상이 있어 수정
			$str = str_replace("/([^(HREF=\"?'?)|(SRC=\"?'?)]|\(|^)((http|https|ftp|telnet|news|mms):\/\/[a-zA-Z0-9\.-]+\.[가-힣\xA1-\xFEa-zA-Z0-9\.:&#=_\?\/~\+%@;\-\|\,\(\)]+)/i", "\\1<A HREF=\"\\2\" TARGET='{$config['cf_link_target']}'>\\2</A>", $str);
	
			// 이메일 정규표현식 수정 061004
			//$str = str_replace("/(([a-z0-9_]|\-|\.)+@([^[:space:]]*)([[:alnum:]-]))/i", "<a href='mailto:\\1'>\\1</a>", $str);
			$str = str_replace("/([0-9a-z]([-_\.]?[0-9a-z])*@[0-9a-z]([-_\.]?[0-9a-z])*\.[a-z]{2,4})/i", "<a href='mailto:\\1'>\\1</a>", $str);
			$str = str_replace("/\t_nbsp_\t/", "&nbsp;" , $str);
			$str = str_replace("/\t_lt_\t/", "&lt;", $str);
			$str = str_replace("/\t_gt_\t/", "&gt;", $str);
			*/
	...
	{	
		...
		if($step > 0) {	
			...
			if($icon == "")
				$depth = $depth . "<img src=\"" . $GP -> IMG_PATH. "/comm_img/reply.png\" border=0 align=\"absmiddle\"> ";
			else
				$depth = $depth . $icon;
		}
	...
	*/
	...
	function ckBrowser() {
		...
		if(preg_match ( 'Mozilla', $agent)) { 
			if(preg_match ('rv',$agent)){
				preg_match_all("/rv:(.*)\)/i",$agent,$match,PREG_SET_ORDER);
				return "Mozilla {$match[0][1]}"; 
			}
		}
		
		if (preg_match ('Chrome', $agent)) return "Chrome";
		if (preg_match ('Safari', $agent)) return "Safari";
		if (preg_match ('Lynx', $agent)) return "Lynx";
		if (preg_match ('LibWWW', $agent)) return "LibWWW";
		if (preg_match ('Konqueror', $agent)) return "Konqueror";
		if (preg_match ('Internet Ninja', $agent)) return "Internet Ninja";
		if (preg_match ('Download Ninja', $agent)) return "Download Ninja";
		if (preg_match ('WebCapture', $agent)) return "WebCapture";
		if (preg_match ('LTH', $agent)) return "LTH";
		if (preg_match ('Gecko', $agent)) return "Gecko";
		if (preg_match ('wget', $agent)) return "Wget command";
	
		if (preg_match ('PSP', $agent)) return "PlayStation Portable";
		if (preg_match ('Symbian', $agent)) return "Symbian PDA";
		if (preg_match ('Nokia', $agent)) return "Nokia PDA";
		if (preg_match ('LGT', $agent)) return "LG Mobile";
		if (preg_match ('mobile', $agent)) return "ETC Mobile";
	...
	*/
	...
	function ckOs() {
		...
		if(!$agent=getenv("HTTP_USER_AGENT")) return 'unknown';
			...
			if (preg_match ('win95', $agent) || preg_match ('windows 95', $agent)) return "Windows 95";
			if (preg_match ('Windows 9x', $agent) || preg_match ('Win 9x 4.90', $agent) || preg_match ('Windows Me', $agent)) return "Windows ME";
		if (preg_match ('Win98', $agent) || preg_match ( 'Windows 98', $agent)) return "Windows 98";	
		if (preg_match ('Windows NT 5.1', $agent) || preg_match ('Windows XP', $agent)) return "Windows XP";	
		if (preg_match ('Windows NT 5.0', $agent) || preg_match ('Windows 2000', $agent)) return "Windows 2000";    
			if (preg_match ('windows NT 5.2', $agent) || preg_match ('Windows 2003', $agent)) return "Windows 2003";			
		if (preg_match ('Windows NT 6.1', $agent) || preg_match ('Windows XP', $agent)) return "Windows 7";
		if (preg_match ('windows NT 6', $agent)) return "Windows Vista";			
		if (preg_match ('Winnt', $agent) || preg_match ('Windows NT', $agent)) return "Windows NT";
		if (preg_match ('windows', $agent)) return "ETC Windows";
			if (preg_match ('Mac', $agent )) {
			if(preg_match ('PowerPC' , $agent)) return "Mac PowerPC";
			if(preg_match ('Macintosh' , $agent)) return "Mac Macintosh";
			if(preg_match ('PowerPC' , $agent)) return "Mac OS X";
			return "ETC Mac";
		}	
		if (preg_match ('Os2', $agent)) return "OS2";
		if (preg_match ('Linux', $agent) || preg_match ('Wget', $agent)) return "Linux";
		if (preg_match ('Unix', $agent)) return "Unix";
		if (preg_match ('Freebsd', $agent)) return "Freebsd";
		...
		if (preg_match ('Googlebot', $agent)) return "GoogleBot";
		if (preg_match ('OmniExplorer', $agent)) return "OmniExplorerBot";
		if (preg_match ('MJ12bot', $agent)) return "majestic12Bot";
		if (preg_match ('ia_archiver', $agent)) return "Alexa(IA Archiver)";
		if (preg_match ('Yandex', $agent)) return "Yandex bot";
		if (preg_match ('Inktomi', $agent)) return "Inktomi Slurp";
		if (preg_match ('Giga', $agent)) return "GigaBot";
		if (preg_match ('Jeeves', $agent)) return "Jeeves bot";
		if (preg_match ('Planetwide', $agent)) return "IBM Planetwide bot";
		if (preg_match ('bot', $agent) || preg_match ('Crawler', $agent) || preg_match ('library', $agent)) return "ETC Robot";
	...
	*/
	...
	function ckBot() {
		...
		if (preg_match ('Giga', $agent)) return "GigaBot";
		if (preg_match ('Jeeves', $agent)) return "Jeeves bot";
		if (preg_match ('cowbot', $agent)) return "NaverBot";
		if (preg_match ('MSNBot', $agent)) return "MSNBot";
		if (preg_match ('Slurp', $agent)) return "Yahoo! Slurp";
		if (preg_match ('EMPAS.ROBOT', $agent)) return "EMPAS.ROBOT";
		if (preg_match ('Daumoa', $agent)) return "DAUMOA";
		if (preg_match ('Twiceler', $agent)) return "Twiceler";
		if (preg_match ('Yeti', $agent)) return "Yeti";
		if (preg_match ('bot', $agent) || preg_match ('Crawler', $agent) || preg_match ('library', $agent)) return "Etc";
	...
	function ckEngine() {
		...
		if (preg_match ('naver', $agent)) return "naver";
		if (preg_match ('google', $agent)) return "google";
		if (preg_match ('paran', $agent)) return "paran";
		if (preg_match ('nate', $agent)) return "nate";
		if (preg_match ('daum', $agent)) return "daum";
		if (preg_match ('bing', $agent)) return "bing";
		if (preg_match ('yahoo', $agent)) return "yahoo";
		if (preg_match ('msn', $agent)) return "msn";
		if (preg_match ('simmani', $agent)) return "simmani";
		if (preg_match ('altavista', $agent)) return "altavista";		
		return 'unknown';
	}
	...
	function xss_clean($data) {
		...
		$data = str_replace('/(&#*\w+)[\x00-\x20]+;/u', '$1;', $data);
		$data = str_replace('/(&#x*[0-9A-F]+);*/iu', '$1;', $data);
		$data = html_entity_decode($data, ENT_COMPAT, 'UTF-8');

		// Remove any attribute starting with "on" or xmlns
		$data = str_replace('#(<[^>]+?[\x00-\x20"\'])(?:on|xmlns)[^>]*+>#iu', '$1>', $data);

		// Remove javascript: and vbscript: protocols
		$data = str_replace('#([a-z]*)[\x00-\x20]*=[\x00-\x20]*([`\'"]*)[\x00-\x20]*j[\x00-\x20]*a[\x00-\x20]*v[\x00-\x20]*a[\x00-\x20]*s[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:#iu', '$1=$2nojavascript...', $data);
		$data = str_replace('#([a-z]*)[\x00-\x20]*=([\'"]*)[\x00-\x20]*v[\x00-\x20]*b[\x00-\x20]*s[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:#iu', '$1=$2novbscript...', $data);
		$data = str_replace('#([a-z]*)[\x00-\x20]*=([\'"]*)[\x00-\x20]*-moz-binding[\x00-\x20]*:#u', '$1=$2nomozbinding...', $data);

		// Only works in IE: <span style="width: expression(alert('Ping!'));"></span>
		$data = str_replace('#(<[^>]+?)style[\x00-\x20]*=[\x00-\x20]*[`\'"]*.*?expression[\x00-\x20]*\([^>]*+>#i', '$1>', $data);
		$data = str_replace('#(<[^>]+?)style[\x00-\x20]*=[\x00-\x20]*[`\'"]*.*?behaviour[\x00-\x20]*\([^>]*+>#i', '$1>', $data);
		$data = str_replace('#(<[^>]+?)style[\x00-\x20]*=[\x00-\x20]*[`\'"]*.*?s[\x00-\x20]*c[\x00-\x20]*r[\x00-\x20]*i[\x00-\x20]*p[\x00-\x20]*t[\x00-\x20]*:*[^>]*+>#iu', '$1>', $data);

		// Remove namespaced elements (we do not need them)
		$data = str_replace('#</*\w+:\w[^>]*+>#i', '', $data);

		do
		{
				// Remove really unwanted tags
				$old_data = $data;
				$data = str_replace('#</*(?:applet|b(?:ase|gsound|link)|embed|frame(?:set)?|i(?:frame|layer)|l(?:ayer|ink)|meta|object|s(?:cript|tyle)|title|xml)[^>]*+>#i', '', $data);
		}
		while ($old_data !== $data);
	...
	function TableExists($tablename, $db) {	
		...
		//$result = mysqli_query("SHOW TABLES from $db"); foreach($list as $key=>$value) echo $value;
	...
	function fnPageCounterUp($user_ip, $sess_id, $request_uri, $referer_url, $page_type)
	{
		...
		//} //end_of_if(preg_match ("^218.234.217", $user_ip))
	}
